...
export default function Anasayfa() {
  ...
  const [allDbCategories, setAllDbCategories] = useState<string[]>([]); // Sitedeki tüm benzersiz kategoriler
  const [homepageCategories, setHomepageCategories] = useState<string[]>([]); // Ana sayfaya sabitlenen 5 kategori
  ...
  const [isSelectorModalOpen, setIsSelectorModalOpen] = useState(false); // 5 Kategori Seçici
  const [selectedCategory, setSelectedCategory] = useState('');
  const [modalImageUrl, setModalImageUrl] = useState('');
  const [oldImageUrlOnServer, setOldImageUrlOnServer] = useState('');
  const [tempSelectedCategories, setTempSelectedCategories] = useState<string[]>([]); // Geçici seçim listesi
  ...
  // 🛠️ 5 Kategori Seçimini Set Etme / Veritabanına Yazma
  ...
  // Kategori Seçim Kutusunu Tetikleme Fonksiyonu
  ...
  // 🛠️ ÖNE ÇIKANLARI SIRALAMA LİSTESİ (sort_onecikan kolonu baz alınır)
  ...
  // 🛠️ YENİ: Sayfa Yenilemeden Anlık (Real-Time) Sıralama Güncelleyen Fonksiyon
  const handleDrop = async (dropIndex: number) => {
    if (!isAdmin || draggedIndex === null || draggedIndex === dropIndex) return;

    // 1. Öne çıkanlar listesindeki elemanların yerini yerel olarak değiştiriyoruz
  ...
  // Eğer henüz sabit kategori seçilmediyse tüm db kategorilerinden ilk 5'ini gösterelim (Fallback)
    ...
    <main className="min-h-screen bg-[#F5F0E6] text-[#3C2F2F] selection:bg-[#5e0d0f] selection:text-white overflow-hidden">
      ...
      <section className="relative h-[85vh] flex items-center justify-center overflow-hidden">
        {heroImages.map((img, index) => (
          <Image 
            key={index} src={img} alt={`Koca Çınar Şarküteri - ${index + 1}`} fill priority={index === 0}
      ...
      {/* === 🛠️ GÜNCELLENEN KATEGORİLER VİTRİNİ (SABİTLEME ÖZELLİKLİ) === */}
      <section className="max-w-7xl mx-auto px-4 sm:px-6 mb-28">
        <div className="flex flex-col sm:flex-row justify-between items-center mb-10 gap-4">
          ...
          {/* Admin Butonu: 5 Kategori Seçici */}
          {isAdmin && (
            <button 
              onClick={() => {
      ...
      {/* === ⚙️ MODAL 1: KATEGORİ SEÇİCİ PANELİ (YENİ ÖZELLİK) === */}
      {isSelectorModalOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white w-full max-w-lg rounded-[28px] p-6 shadow-2xl border border-[#D4A373]/20 flex flex-col gap-4 max-h-[85vh] overflow-y-auto no-scrollbar">
                ...
                return (
                  <button
                    ...
                    className={`p-3 rounded-xl border text-xs font-bold transition-all text-left flex items-center justify-between ${
                      isSelected ? 'bg-[#5e0d0f] text-white border-[#5e0d0f]' : 'bg-gray-50 text-[#3C2F2F] border-gray-200 hover:bg-gray-100'
                    }`}
                  >
                    <span>{cat}</span>
            ...
            <div className="text-xs font-bold text-gray-500 bg-[#F5F0E6] p-3 rounded-xl border border-[#D4A373]/20">
              Seçilen Adet: <span className="text-[#5e0d0f] font-extrabold">{tempSelectedCategories.length} / 5</span>
            </div>
            ...
            <div className="flex gap-2 justify-end pt-3 border-t">
              ...
              <button onClick={handleSaveHomepageCategories} className="bg-[#5e0d0f] text-white px-5 py-2.5 rounded-xl font-bold text-xs hover:bg-[#D4A373] shadow-md transition-all active:scale-95">Kategorileri Kilitle 🔒</button>
            </div>
          </div>
        </div>
      )}

      {/* === ⚙️ MODAL 2: KAPAK GÖRSELİ DÜZENLEME MODALI === */}
      {isCategoryModalOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
